...
function ResultSharePage() {
    ...
    const bodyType = searchParams.get('bodyType') || 'natural';
    ...
    return (
        <div className="min-h-screen w-full bg-gradient-to-br from-emerald-200 via-violet-100 to-cyan-100 px-5 py-8">
            <div className="max-w-[900px] mx-auto rounded-[28px] bg-white/70 backdrop-blur-md shadow-2xl p-6">
                <h1 className="text-center text-2xl font-extrabold text-gray-900 mb-6">
                    체형 분석 기반 코디 추천 결과
                </h1>

                <div className="rounded-2xl bg-white/70 p-5 shadow-md mb-6">
                    <h2 className="text-2xl font-extrabold text-gray-900 mb-3">
                        당신의 Body 타입은 <span className="text-purple-600">{bodyTypeText}</span>입니다
                    </h2>

                    <div className="text-sm font-bold text-gray-700 space-y-1">
                        <p>성별: {gender === 'male' ? '남성' : '여성'}</p>
                        {height && <p>키: {height}cm</p>}
                        {weight && <p>몸무게: {weight}kg</p>}
                    </div>
                </div>

                <div className="space-y-4 mb-6">
                    <section className="rounded-2xl bg-white/75 p-4 shadow-sm">
                        <h3 className="text-lg font-extrabold text-gray-900 mb-2">숄더 라인 및 상체 분석</h3>
                        <p className="text-sm leading-relaxed font-semibold text-gray-800">{bodyComment.shoulder}</p>
                    </section>

                    <section className="rounded-2xl bg-white/75 p-4 shadow-sm">
                        <h3 className="text-lg font-extrabold text-gray-900 mb-2">하체 및 비율 분석</h3>
                        <p className="text-sm leading-relaxed font-semibold text-gray-800">{bodyComment.lower}</p>
                    </section>

                    <section className="rounded-2xl bg-white/75 p-4 shadow-sm">
                        <h3 className="text-lg font-extrabold text-gray-900 mb-2">코디네이션 전략</h3>
                        <p className="text-sm leading-relaxed font-semibold text-gray-800">{bodyComment.styling}</p>
                    </section>
                </div>

                <h2 className="text-xl font-extrabold text-gray-900 mb-4">추천 코디</h2>

                {!selectedOutfit ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        {otherOutfits.map((item) => (
                            <button
                                key={item.id}
                                onClick={() => setSelectedOutfit(item)}
                                className="rounded-2xl bg-white/80 shadow-md p-4 text-left active:scale-[0.98] transition"
                            >
                                <h3 className="text-center text-lg font-extrabold text-gray-900 mb-3">
                                    [{item.title}]
                                </h3>

                                <div className="h-[260px] rounded-xl bg-white flex items-center justify-center overflow-hidden shadow-inner">
                    ...
                    <div className="rounded-2xl bg-white/90 shadow-xl p-5">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-xl font-extrabold text-gray-900">[{selectedOutfit.title}]</h3>

                            <button
                                onClick={() => setSelectedOutfit(null)}
                                className="w-9 h-9 rounded-full bg-gray-200 text-xl font-extrabold"
                            >
                                ×
                            </button>
                        </div>

                        <div className="h-[300px] rounded-2xl bg-gradient-to-br from-purple-100 to-cyan-100 flex items-center justify-center overflow-hidden shadow-inner mb-5">
                            <img
                        ...
                        <div className="rounded-2xl bg-gradient-to-br from-emerald-100/80 to-violet-100/80 p-4">
                            <h4 className="text-lg font-extrabold text-gray-900 mb-3">스타일 설명</h4>

                            <p className="text-sm leading-relaxed font-semibold text-gray-800 whitespace-pre-line">
                                {selectedOutfit.desc}
                            </p>

                            <div className="flex flex-wrap gap-2 mt-5">
                                <span className="px-3 py-1 rounded-full bg-white/80 text-purple-600 font-bold text-sm">
                                    #체형보완
                                </span>
                                <span className="px-3 py-1 rounded-full bg-white/80 text-purple-600 font-bold text-sm">
                                    #비율보정
